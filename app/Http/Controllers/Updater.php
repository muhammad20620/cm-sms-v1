...
class Updater extends Controller
{
    function update(Request $request){
        ...
        if ($validator->fails()){
            return redirect()->back()->with('warning', get_phrase('Select a valid zip file'));
        }
        ...
        if (class_exists('ZipArchive')) {
            ...
            return redirect()->back()->with('warning', get_phrase('Your server is unable to extract the zip file').'. '.get_phrase('Please enable the zip extension to the server, then try again'));
        }
		...
		if(array_key_exists('is_addon', $config) && strval($config['is_addon']) == "1"){
			...
			if(str_contains($product_current_version, '.10')){
				$product_current_version = str_replace('.10', '.9.1', $product_current_version);
			}
			if(str_contains($product_current_version, '.10')){
				$product_current_version = str_replace('.20', '.19.1', $product_current_version);
			}
			...
			if($config['product_version']['minimum_required_version'] > $product_current_version){
				return redirect()->back()->with('warning', get_phrase("You have to update your main application's version.").'('.$config['product_version']['minimum_required_version'].') '.get_phrase(' to install the addon'));
			}
			$product_current_version = $pre_version;
			...
			if(strval($config['addon_version']['minimum_required_version']) > strval($addons_current_version)){
				return redirect()->back()->with('warning', get_phrase('It looks like you are skipping a version').'. '.get_phrase('Please update version').' '.$config['addon_version']['minimum_required_version'].' '.get_phrase('first'));
			}
			...
			if(strval($config['product_version']['minimum_required_version']) != strval($product_current_version)){
				return redirect()->back()->with('warning', get_phrase('It looks like you are skipping a version').'. '.get_phrase('Please update version').' '.$config['product_version']['minimum_required_version'].' '.get_phrase('first'));
			}
		}

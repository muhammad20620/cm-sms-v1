    ...
    <div class="row">
        <div class="col-12">
            <div class="eSection-wrap-2">
                <div
                    class="search-filter-area d-flex justify-content-md-between justify-content-center align-items-center gr-15">
                    <form action="{{ route('admin.admin') }}">
                        <div class="search-input d-flex justify-content-start align-items-center"
                            ...
                            <input type="text" id="search" name="search" value="{{ $search }}"
                                placeholder="Search Admin" class="form-control" />
                        </div>
                    </form>
                    <!-- Export Button -->
                    @if (count($admins) > 0)
                ...
                @if (count($admins) > 0)
                    ...
                    <div class="table-responsive">
                                        ...
                                        <td>
                                            <div class="dAdmin_info_name min-w-100px">
                                                @if (!empty($admin->account_status == 'disable'))
                                                    <span
                                                        class="eBadge ebg-soft-danger">{{ get_phrase('Disabled') }}</span>
                                                @else
                                        ...
                                        <td>
                                            <div class="adminTable-action">
                                                ...
                                                <ul
                                                        ...
                                                        @if (auth()->user()->school_role == 1)
                                                            <li>
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.admin.menu_permission', ['id' => $admin->id]) }}">{{ get_phrase('Admin Permission') }}</a>
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item" href="javascript:;"
                                                            onclick="largeModal('{{ route('admin.admin.admin_profile', ['id' => $admin->id]) }}','{{ get_phrase('Admin Profile') }}')">{{ get_phrase('Profile') }}</a>
                                                    </li>
                                                    <li>
                                                        <a class="dropdown-item"
                                                            href="{{ route('admin.documents', ['id' => $admin->id]) }}">{{ get_phrase('Documents') }}</a>
                                                    </li>
                                                    @if (auth()->user()->id != $admin->id && $key != 0)
                                                        @if ($admin->account_status == 'disable')
                                                            <li>
                                                                <a class="dropdown-item" href="javascript:;"
                                                                    onclick="confirmModal('{{ route('admin.account_enable', ['id' => $admin->id]) }}', 'undefined');">{{ get_phrase('Enable') }}</a>
                                                            </li>
                                                        @else
                                                            <li>
                                                                <a class="dropdown-item" href="javascript:;"
                                                                    onclick="confirmModal('{{ route('admin.account_disable', ['id' => $admin->id]) }}', 'undefined');">{{ get_phrase('Disable') }}</a>
                                                            </li>
                                                        @endif
                                                    @endif
                                                </ul>
                        ...
                        <div
                            ...
                            <p class="admin-tInfo">
                                {{ get_phrase('Showing') . ' 1 - ' . count($admins) . ' ' . get_phrase('from') . ' ' . $admins->total() . ' ' . get_phrase('data') }}
                            </p>
